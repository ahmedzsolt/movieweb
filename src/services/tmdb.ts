...
export async function getMovieProvidersInCountry(region: string) {
  ...
  if (!response.ok) {
    throw new Error(
      "Fetch for getting movie providers for a country gone wrong.",
    );
  }
}
...
export async function getProviders(movie: Movie, region: string) {
  ...
  if (
    result.results[region] === undefined ||
    result.results[region]["flatrate"] === undefined
  ) {
    return [];
  }

  const providerIds: number[] = result.results[region]["flatrate"].map(
...
export async function getProvidersForMovie(movie: Movie, region: string) {
  ...
  if (!response.ok) {
    throw new Error(
      "Fetch gone wrong for getting movie providers for chosen movie.",
    );
  }
...
async function isMovieProvidedByProvider(
  movie: Movie,
  region: string,
  providers: Provider[],
) {
...
export async function getRecommendations(
  ...
  for (const recMovie of result.results) {
    const isMovieChosenBefore = chosenMovies.some(
      (chosenMovie) => chosenMovie.id === recMovie.id,
    );
    ...
    const isMovieProvided = await isMovieProvidedByProvider(
      recMovie,
      region,
      providers,
    );

...
function MovieCard({movie, setImagesLoaded}: MovieCardProps) {
    ...
    return (
        <button onClick={() => setChosenMovies(prev => [...prev, movie])} className="flex flex-col basis-1/5 cursor-pointer">
            <img src={"https://image.tmdb.org/t/p/original/" + movie.poster_path} onLoad={updateImagesLoaded} onError={updateImagesLoaded} className="object-cover max-h-24 md:max-h-100"/>
